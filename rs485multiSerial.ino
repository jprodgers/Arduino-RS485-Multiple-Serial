...
115200  173
*/
#define BAUDRATE 115200 // Slower is generally better, but I get good results here.
...
#define PORT1_ENABLE 8 // Connected to pins 2&3 of the 1st MAX485
#define PORT1_TX 3     // Connected to pin1 of the 1st MAX485
#define PORT1_RX 4     // Connected to pin4 of the 1st MAX485

#define PORT2 2        // Used to identify the port in code
#define PORT2_ENABLE 9 // Connected to pins 2&3 of the 2nd MAX485
#define PORT2_TX 5     // Connected to pin1 of the 2nd MAX485
#define PORT2_RX 6     // Connected to pin4 of the 2nd MAX485
...
// Stuff to change
boolean master = true;       // master receives data only
...
int sendReceiveTimeout = 30; // timeout before send or receives gives up
int sendDelay = 500;         // Time between the slave sending data

void setup() {
  Serial.begin(BAUDRATE);
  Serial.println("Initializing Software Serial");
  ...
  if (debug) eccoTest(PORT1); // Loops, just sending data back and forth.
    ...
    */
    ...
    for (int count = 0; count < 255; count++) {
      ...
      Serial.print("Number of bytes being sent: ");
      Serial.println(qty);
      Serial.print("Sending attempt: ");
      Serial.println(count);
      if (sendData(finalData, qty, PORT1)) {
      ...
      */
      Serial.println("Ratio sent/failed");
...
boolean sendData(const byte *data, int numBytes, byte port) {
  ...
  Serial.println("Starting sendData");
  Serial.println("Constructing data");
  ...
  //delay(sendReceiveTimeout / 4); // lets the buffer clear, and receiver to verify
  long timeOut = millis();
  Serial.println("Receiving Verification Data");
...
boolean receiveData(byte *data, byte maxDataLength, byte port) {
  if (checkPortHasData(port)) {
    ...
    if (verboseMode) Serial.println("Retrieving data: ");
    byte count = readBuffer(tempData, maxDataLength, port);
    if (count > 0) {
      Serial.print("Received Count of ");
      Serial.println(count);
    }
    else return false;
    Serial.println("Data Recieved:");
    if (verboseMode) serialPrintBuffer(tempData, count);
    Serial.println("Resending Data: ");
    if (verboseMode) serialPrintBuffer(tempData, count);
    sendBuffer(tempData, count, port);
    //delay(sendReceiveTimeout / 4);
      ...
      byte response = readByte(port);
        ...
        if (response > 0) {
          ...
          if (response == ACK) {
            ...
            Serial.println("Data Failed Verification");
            return false;
          }
          else return false;
        }
    ...
    Serial.println("Timeout/Failout");
    return false;
  }
  else return false;
}
  ...
  while (timeOut + sendReceiveTimeout > millis()) {
    if (checkPortHasData(port)) {
      while (count < maxDataLength) {
        if (port == PORT1) {
          while (count < maxDataLength + 1 && rs485Port1.available()) {
            tempData[count++] = rs485Port1.read();
          }
        }
        else if (port == PORT2) {
          while (count < maxDataLength + 1 && rs485Port2.available()) {
...
void sendBuffer(const byte *data, byte numBytes, byte port) {
  if (port == PORT1) {
    ...
    Serial.println("Sending Data on Port1");
    for (int i = 0; i < numBytes; i++) rs485Port1.write(data[i]);
    //rs485Port1.flush();
    delayMicroseconds(readWriteDelay);
    digitalWrite(PORT1_ENABLE, RECEIVE_MODE);
  }
    ...
    Serial.println("Sending Data on Port2");
    for (int i = 0; i < numBytes; i++) rs485Port2.write(data[i]);
    //rs485Port2.flush();
    delayMicroseconds(readWriteDelay);
    digitalWrite(PORT2_ENABLE, RECEIVE_MODE);
  }
